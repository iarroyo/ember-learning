...
module('Integration | Component | search-box', function (hooks) {
  ...
  test('it renders with default placeholder', async function (assert) {
    ...
    assert.dom('[data-test-search-input]').hasAttribute('placeholder', 'Search...');
  });
  ...
  test('it renders with custom placeholder', async function (assert) {
    ...
    assert.dom('[data-test-search-input]').hasAttribute('placeholder', 'Find products...');
  });
  ...
  test('uses lastSuccessful to show stale data while loading', async function (assert) {
    ...
    fillIn('[data-test-search-input]', 'second');
    await waitFor('[data-test-search-loading]');
    ...
    await waitUntil(() => document.querySelector('[data-test-search-result]')?.textContent?.includes('second'), { timeout: 2000 });
  ...
  test('two SearchBox components have independent tasks', async function (assert) {
    ...
    fillIn('[data-test-box-1] [data-test-search-input]', 'query1');
    fillIn('[data-test-box-2] [data-test-search-input]', 'query2');
    ...
    await waitUntil(() => !document.querySelector('[data-test-box-2] [data-test-search-loading]'), { timeout: 2000 });
  });
});
